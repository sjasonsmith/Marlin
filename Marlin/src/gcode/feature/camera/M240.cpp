...
#include "../../gcode.h"
#include "../../../module/motion.h" // for active_extruder and motion.current_position_rw()
...
void GcodeSuite::M240() {
    ...
    const xyz_pos_t old_pos = {
      motion.current_position_rw().x + parser.linearval('A'),
      motion.current_position_rw().y + parser.linearval('B'),
      motion.current_position_rw().z
    };
    ...
    xyz_pos_t raw = {
      ...
      (parser.seenval('Z') ? parser.value_linear_units() : photo_position.z) + motion.current_position_rw().z
    };
