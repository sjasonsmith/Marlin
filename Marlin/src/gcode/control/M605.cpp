  ...
  void GcodeSuite::M605() {
    ...
    if (parser.seen('S')) {
      ...
      if (dual_x_carriage_mode == DXC_MIRRORED_MODE) {
        ...
        float x_jog = motion.current_position_rw().x - .1;
        for (uint8_t i = 2; --i;) {
          planner.buffer_line(x_jog, motion.current_position_rw().y, motion.current_position_rw().z, motion.current_position_rw().e, feedrate_mm_s, 0);
          x_jog += .1;
        }
        return;
      }
      ...
      if (parser.seen('W')) {
        ...
        DEBUG_ECHOPAIR("\nactive_extruder_x_pos: ", motion.current_position_rw().x);
