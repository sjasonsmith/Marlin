...
void GcodeSuite::M48() {
  ...
  const xy_pos_t test_position = {
    parser.linearval('X', motion.current_position_rw().x + probe.offset_xy.x),  // If no X use the probe's current X position
    parser.linearval('Y', motion.current_position_rw().y + probe.offset_xy.y)   // If no Y, ditto
  };
