...
G29_TYPE GcodeSuite::G29() {
  ...
  if (!isnan(measured_z)) {
      ...
      if (!dryrun) {
        ...
        // The rotated XY and corrected Z are now motion.current_position()
        motion.current_position() = converted;
      ...
      if (!dryrun) {
        ...
        motion.current_position().z -= fade_scaling_factor * bilinear_z_offset(motion.current_position());
