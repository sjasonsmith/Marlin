...
void GcodeSuite::G42() {
  if (MOTION_CONDITIONS) {
    ...
    if (hasI) motion.destination_rw().x = _GET_MESH_X(ix);
    if (hasJ) motion.destination_rw().y = _GET_MESH_Y(iy);

    #if HAS_PROBE_XY_OFFSET
      if (parser.boolval('P')) {
        if (hasI) motion.destination_rw().x -= probe.offset_xy.x;
        if (hasJ) motion.destination_rw().y -= probe.offset_xy.y;
      }
    #endif
