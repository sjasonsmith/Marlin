...
inline void fast_line_to_current(const AxisEnum fr_axis) { _line_to_current(fr_axis); }
  ...
  inline void magnetic_parking_extruder_tool_change(const uint8_t new_tool) {
    ...
    motion.current_position().x = mpe_settings.parking_xpos[new_tool] + offsetcompensation;

    DEBUG_ECHOPAIR("(1) Move extruder ", int(new_tool));
    DEBUG_POS(" to new extruder ParkPos", motion.current_position());

    planner.buffer_line(motion.current_position(), mpe_settings.fast_feedrate, new_tool);
    planner.synchronize();

    // STEP 2

    motion.current_position().x = grabpos + offsetcompensation;

    DEBUG_ECHOPAIR("(2) Couple extruder ", int(new_tool));
    DEBUG_POS(" to new extruder GrabPos", motion.current_position());

    planner.buffer_line(motion.current_position(), mpe_settings.slow_feedrate, new_tool);
    planner.synchronize();
    ...
    motion.current_position().x = mpe_settings.parking_xpos[active_extruder] + (active_extruder == 0 ? MPE_TRAVEL_DISTANCE : -MPE_TRAVEL_DISTANCE) + offsetcompensation;
    ...
    motion.current_position().x = mpe_settings.parking_xpos[active_extruder] + offsetcompensation;
    ...
    motion.current_position().x = oldx;
  ...
  inline void parking_extruder_tool_change(const uint8_t new_tool, bool no_move) {
    if (!no_move) {
      ...
      motion.current_position().x = parkingposx[active_extruder] + x_offset;
      ...
      motion.current_position().x += active_extruder ? -10 : 10; // move 10mm away from parked extruder
      ...
      motion.current_position().x = grabpos + (new_tool ? -10 : 10);
      fast_line_to_current(X_AXIS);

      motion.current_position().x = grabpos;
      ...
      motion.current_position().x = midpos - TERN0(HAS_HOTEND_OFFSET, hotend_offset[new_tool].x);
  ...
  inline void switching_toolhead_tool_change(const uint8_t new_tool, bool no_move/*=false*/) {
    ...
    motion.current_position().x = placexpos;
    ...
    motion.current_position().y = SWITCHING_TOOLHEAD_Y_POS - (SWITCHING_TOOLHEAD_Y_SECURITY);
    ...
    motion.current_position().y = SWITCHING_TOOLHEAD_Y_POS;
    ...
    motion.current_position().y -= SWITCHING_TOOLHEAD_Y_CLEAR;
    DEBUG_POS("Move back Y clear", motion.current_position());
    fast_line_to_current(Y_AXIS); // move away from docked toolhead

    // 3. Move to the new toolhead

    motion.current_position().x = grabxpos;
  ...
  inline void magnetic_switching_toolhead_tool_change(const uint8_t new_tool, bool no_move/*=false*/) {
    ...
    motion.current_position().y = SWITCHING_TOOLHEAD_Y_POS + SWITCHING_TOOLHEAD_Y_CLEAR;
    ...
    motion.current_position().x = placexclear;
    ...
    motion.current_position().y = SWITCHING_TOOLHEAD_Y_POS + SWITCHING_TOOLHEAD_Y_RELEASE;
    ...
    _line_to_current(Y_AXIS, 0.2f);
        ...
        motion.current_position().e += SWITCHING_TOOLHEAD_PRIME_MM;
        planner.buffer_line(motion.current_position(), MMM_TO_MMS(SWITCHING_TOOLHEAD_PRIME_FEEDRATE), new_tool);
      #endif
      #if SWITCHING_TOOLHEAD_RETRACT_MM
        motion.current_position().e -= SWITCHING_TOOLHEAD_RETRACT_MM;
        planner.buffer_line(motion.current_position(), MMM_TO_MMS(SWITCHING_TOOLHEAD_RETRACT_FEEDRATE), new_tool);
      #endif
    #else
      planner.synchronize();
    ...
    motion.current_position().x = grabxclear;
  ...
  inline void em_switching_toolhead_tool_change(const uint8_t new_tool, bool no_move) {
    ...
    motion.current_position().z += SWITCHING_TOOLHEAD_Z_HOP;
    ...
    motion.current_position().set(hoffs.x + placexpos,
    ...
    motion.current_position().y += SWITCHING_TOOLHEAD_Y_CLEAR;
    slow_line_to_current(Y_AXIS);
    motion.current_position().set(hoffs.x + grabxpos,
                         hoffs.y + SWITCHING_TOOLHEAD_Y_POS + SWITCHING_TOOLHEAD_Y_CLEAR);
    fast_line_to_current(X_AXIS);
    ...
    motion.current_position().z += hoffs.z - hotend_offset[new_tool].z;
  ...
  inline void dualx_tool_change(const uint8_t new_tool, bool &no_move) {
    ...
    if (dual_x_carriage_mode == DXC_AUTO_PARK_MODE                  // If Auto-Park mode is enabled
      ...
      motion.current_position().x = xhome;
...
void tool_change_prime() {
  if (toolchange_settings.extra_prime > 0
    ...
    if (ok) {
      ...
      motion.current_position().z += toolchange_settings.z_raise;
      #if HAS_SOFTWARE_ENDSTOPS
        NOMORE(motion.current_position().z, soft_endstop.max.z);
      #endif
      fast_line_to_current(Z_AXIS);
      planner.synchronize();
    }
    ...
    // Park
      ...
      if (ok) {
        IF_DISABLED(TOOLCHANGE_PARK_Y_ONLY, motion.current_position().x = toolchange_settings.change_point.x);
        IF_DISABLED(TOOLCHANGE_PARK_X_ONLY, motion.current_position().y = toolchange_settings.change_point.y);
        planner.buffer_line(motion.current_position(), MMM_TO_MMS(TOOLCHANGE_PARK_XY_FEEDRATE), active_extruder);
        planner.synchronize();
      }
    #endif
...
void tool_change(const uint8_t new_tool, bool no_move/*=false*/) {
    ...
    if (new_tool != old_tool) {
      ...
      // Toolchange park
        ...
        if (can_move_away && toolchange_settings.enable_park) {
          IF_DISABLED(TOOLCHANGE_PARK_Y_ONLY, motion.current_position().x = toolchange_settings.change_point.x);
          IF_DISABLED(TOOLCHANGE_PARK_X_ONLY, motion.current_position().y = toolchange_settings.change_point.y);
          planner.buffer_line(motion.current_position(), MMM_TO_MMS(TOOLCHANGE_PARK_XY_FEEDRATE), old_tool);
          planner.synchronize();
        }
      #endif
        ...
        if (!no_move) {
          ...
          motion.current_position().z = _MIN(newz + toolchange_settings.z_raise, maxz);
          fast_line_to_current(Z_AXIS);
        }
  ...
  bool extruder_migration() {
    ...
    motion.current_position().e = resume_current_e;
