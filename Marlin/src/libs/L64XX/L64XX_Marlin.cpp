...
uint8_t L64XX_Marlin::get_user_input(uint8_t &driver_count, L64XX_axis_t axis_index[3],  char axis_mon[3][3],
  ...
  const float X_center = LOGICAL_X_POSITION(motion.current_position_rw().x),
              Y_center = LOGICAL_Y_POSITION(motion.current_position_rw().y),
              Z_center = LOGICAL_Z_POSITION(motion.current_position_rw().z),
              E_center = motion.current_position_rw().e;
