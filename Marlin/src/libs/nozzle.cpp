...
#include "../module/motion.h"
  ...
  void Nozzle::stroke(const xyz_pos_t &start, const xyz_pos_t &end, const uint8_t &strokes) {
    TERN_(NOZZLE_CLEAN_GOBACK, const xyz_pos_t oldpos = motion.current_position_rw());
  ...
  void Nozzle::zigzag(const xyz_pos_t &start, const xyz_pos_t &end, const uint8_t &strokes, const uint8_t &objects) {
    ...
    TERN_(NOZZLE_CLEAN_GOBACK, const xyz_pos_t back = motion.current_position_rw());
  ...
  void Nozzle::clean(const uint8_t &pattern, const uint8_t &strokes, const float &radius, const uint8_t &objects, const uint8_t cleans) {
    ...
    if (pattern == 2) {
      ...
      if (!TEST(cleans, X_AXIS)) start[arrPos].x = end[arrPos].x = motion.current_position_rw().x;
      if (!TEST(cleans, Y_AXIS)) start[arrPos].y = end[arrPos].y = motion.current_position_rw().y;
    }
    if (!TEST(cleans, Z_AXIS)) start[arrPos].z = end[arrPos].z = motion.current_position_rw().z;
  ...
  void Nozzle::park(const uint8_t z_action, const xyz_pos_t &park/*=NOZZLE_PARK_POINT*/) {
    ...
    switch (z_action) {
      ...
      case 2: // Raise by Z-park height
        do_blocking_move_to_z(_MIN(motion.current_position_rw().z + park.z, Z_MAX_POS), fr_z);
        break;

      default: {
        // Apply a minimum raise, overriding G27 Z
        const float min_raised_z =_MIN(Z_MAX_POS, motion.current_position_rw().z
    ...
    do_blocking_move_to_xy(
      TERN(NOZZLE_PARK_Y_ONLY, motion.current_position_rw(), park).x,
      TERN(NOZZLE_PARK_X_ONLY, motion.current_position_rw(), park).y,
      fr_xy
    );
