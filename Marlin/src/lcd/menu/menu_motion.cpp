...
static void _lcd_move_xyz(PGM_P const name, const AxisEnum axis) {
  ...
  if (ui.encoderPosition && !ui.manual_move.processing) {
    ...
    #if IS_KINEMATIC
      // TODO JASON: This uses an offset for kinematic so that it will end up doing more than a single move
    ...
    #else
      // TODO JASON: Updates current position for non-kinematic and assumes it won't end up neededing current_position later.
  ...
  static void lcd_move_e(TERN_(MULTI_MANUAL, const int8_t eindex=-1)) {
    ...
    if (ui.encoderPosition) {
      if (!ui.manual_move.processing) {
        const float diff = float(int32_t(ui.encoderPosition)) * ui.manual_move.menu_scale;
        TERN(IS_KINEMATIC, ui.manual_move.offset, current_position.e) += diff; // TODO JASON: Another place where these manual moves differ
