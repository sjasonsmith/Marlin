...
void probe_offset_wizard_menu() {
  START_MENU();
  calculated_z_offset = probe.offset.z + motion.current_position_rw().z - z_offset_ref;

  if (LCD_HEIGHT >= 4)
    STATIC_ITEM(MSG_MOVE_NOZZLE_TO_BED, SS_CENTER|SS_INVERT);

  STATIC_ITEM_P(PSTR("Z="), SS_CENTER, ftostr42_52(motion.current_position_rw().z));
  ...
  ACTION_ITEM(MSG_BUTTON_DONE, []{
    ...
    motion.current_position_rw().z = z_offset_ref;  // Set Z to z_offset_ref, as we can expect it is at probe height
    sync_plan_position();
...
void prepare_for_probe_offset_wizard() {
  ...
  ui.wait_for_move = true;
  motion.current_position_rw() += probe.offset_xy;
