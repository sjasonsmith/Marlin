...
void MarlinUI::draw_status_screen() {
  ...
  tft_string.set(blink && not_homed ? "?" : ftostr4sign(LOGICAL_X_POSITION(motion.current_position_rw().x)));
  ...
  tft_string.set(blink && not_homed ? "?" : ftostr4sign(LOGICAL_Y_POSITION(motion.current_position_rw().y)));
  ...
  if (blink && not_homed)
    ...
    const float z = LOGICAL_Z_POSITION(motion.current_position_rw().z);
...
void MenuEditItemBase::draw_edit_screen(PGM_P const pstr, const char* const value/*=nullptr*/) {
  ...
  tft.add_text(tft_string.center(TFT_WIDTH), MENU_TEXT_Y_OFFSET, COLOR_MENU_VALUE, tft_string);
    ...
    if (ui.external_control) {
      ...
      tft_string.set(ftostr52(LOGICAL_X_POSITION(motion.current_position_rw().x)));
      tft_string.trim();
      ...
      tft_string.set(ftostr52(LOGICAL_X_POSITION(motion.current_position_rw().y)));
      tft_string.trim();
...
static void drawAxisValue(AxisEnum axis) {
  const float value =
    ...
    #endif
    NATIVE_TO_LOGICAL(
      ui.manual_move.processing ? motion.destination_rw()[axis] : motion.current_position_rw()[axis] + TERN0(IS_KINEMATIC, ui.manual_move.offset),
      axis
    );
  xy_int_t pos;
  uint16_t color;
...
static void moveAxis(AxisEnum axis, const int8_t direction) {
  ...
  if (axis == Z_AXIS && motionAxisState.z_selection == Z_SELECTION_Z_PROBE) {
      ...
      probe.offset.z += diff;
      if (direction < 0 && motion.current_position_rw()[axis] < Z_PROBE_OFFSET_RANGE_MIN) {
        motion.current_position_rw()[axis] = Z_PROBE_OFFSET_RANGE_MIN;
        drawMessage(GET_TEXT(MSG_LCD_SOFT_ENDSTOPS));
      }
      else if (direction > 0 && motion.current_position_rw()[axis] > Z_PROBE_OFFSET_RANGE_MAX) {
        motion.current_position_rw()[axis] = Z_PROBE_OFFSET_RANGE_MAX;
        drawMessage(GET_TEXT(MSG_LCD_SOFT_ENDSTOPS));
      }
      else {
        drawMessage(""); // clear the error
  ...
  if (!ui.manual_move.processing) {
    ...
    // Get the new position
      ...
      if (direction < 0 && motion.current_position_rw()[axis] < min) {
        motion.current_position_rw()[axis] = min;
        drawMessage(GET_TEXT(MSG_LCD_SOFT_ENDSTOPS));
      }
      else if (direction > 0 && motion.current_position_rw()[axis] > max) {
        motion.current_position_rw()[axis] = max;
        drawMessage(GET_TEXT(MSG_LCD_SOFT_ENDSTOPS));
      }
      else {
        drawMessage(""); // clear the error
