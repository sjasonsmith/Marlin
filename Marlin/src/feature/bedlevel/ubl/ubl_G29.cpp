...
#include "../../../inc/MarlinConfig.h"
  ...
  void unified_bed_leveling::shift_mesh_height() {
    ...
    void unified_bed_leveling::move_z_with_encoder(const float &multiplier) {
      ...
      while (!ui.button_pressed()) {
        ...
        if (encoder_diff) {
          do_blocking_move_to_z(motion.current_position().z + float(encoder_diff) * multiplier);
          encoder_diff = 0;
        }
      }
    }
    ...
    float unified_bed_leveling::measure_business_card_thickness() {
      ...
      do_blocking_move_to_z(motion.current_position().z + SIZE_OF_LITTLE_RAISE);
      planner.synchronize();
      ...
      do_blocking_move_to_z(motion.current_position().z + Z_CLEARANCE_BETWEEN_PROBES);
    ...
    void unified_bed_leveling::manually_probe_remaining_mesh(const xy_pos_t &pos, const float &z_clearance, const float &thick, const bool do_ubl_mesh_map) {
      ...
      do {
        ...
        z_values[lpos.x][lpos.y] = motion.current_position().z - thick;
  ...
  bool unified_bed_leveling::g29_parameter_parsing() {
    ...
    float sx = xy_seen.x ? parser.value_float() : motion.current_position().x;
    xy_seen.y = parser.seenval('Y');
    float sy = xy_seen.y ? parser.value_float() : motion.current_position().y;
  ...
  mesh_index_pair unified_bed_leveling::find_closest_mesh_point_of_type(const MeshPointType type, const xy_pos_t &pos, const bool probe_relative/*=false*/, MeshFlags *done_flags/*=nullptr*/) {
    ...
    GRID_LOOP(i, j) {
      if ( (type == (isnan(z_values[i][j]) ? INVALID : REAL))
        ...
        const xy_pos_t diff = motion.current_position() - mpos;
